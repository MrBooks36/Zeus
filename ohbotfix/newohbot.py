...
if not path.exists(ohbotMotorDefFile):
    ...
    if debug:
        print("Copied MotorDefinitionsOhbot.omd from :" + directory + " to ohbotData/")
...
if not path.exists(soundFolder):
    ...
    if debug:
        print("Copied Sounds from :" + directory + " to OhbotData/")
...
def _loadMotorDefs():
    tree = etree.parse(ohbotMotorDefFile)
    root = tree.getroot()
...
# Function to make Ohbot Speak. Arguments | text String "Hello World" **eg say("Hello my name is Ohbot")
...
# Function to play sounds located in the ohbotData/Sounds/ folder.
